...
Read [Survey cycles and table names](@ref) for how a year maps onto a table
name, and the [API reference](@ref) for the full call surface.
...
"Don't know", and similar) stay numeric along with the rest of the column, so an
unguarded `mean` includes them. Call `codebook` to see which codes a variable
reserves, and filter them before summarising.

```julia
demo = NHANES.download("DEMO_J"; translate = false)   # keep numeric codes
NHANES.translate!(demo, "DEMO_J", :RIAGENDR)          # translate one column in place
...
Metadata caches expire after 30 days. Both live under
`~/.julia/scratchspaces/`.
